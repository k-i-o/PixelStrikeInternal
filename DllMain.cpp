...
{
	while (!Utils::KeyPressed(VK_F9))
	{
		void* IL2CPPPlayerCacheThread = IL2CPP::Thread::Attach(IL2CPP::Domain::Get());
		Utils::PlayerList.clear();

		auto RunningList = Unity::Object::FindObjectsOfType<Unity::CComponent>("Health");
		...
		Utils::LocalPlayer = Unity::Object::FindObjectOfType<Unity::CComponent>("PlayerController")->GetGameObject();
...
QWORD WINAPI MainThread(LPVOID param)
{
	...
	while (!Utils::KeyPressed(VK_F9)) // uninject
